...
import com.caitu99.lsp.parser.travel.yazhouwanlitong.Yazhouwanlitong;
import com.caitu99.lsp.utils.SpringContext;

@Service("YazhouwanlitongNormalCard")
public class NormalCard extends Yazhouwanlitong implements ICard{
	
	@Autowired
    private AppConfig appConfig;

    @Value("${yazhouwanlitong.normal.id}")
    private Integer id;

	@Value("${yazhouwanlitong.normal.name}")
    private String name;

    @Value("${yazhouwanlitong.normal.bill.type}")
    private int billType;

    @Value("${yazhouwanlitong.normal.mail.key}")
    private String titleKey;

    @Value("${yazhouwanlitong.normal.mail.key2}")
    private String titleKey2;

    @Value("${yazhouwanlitong.normal.mail.sender}")
    private String mailSender;

    @Value("${yazhouwanlitong.normal.history.bill}")
    ...
    public boolean is(String title, String sender) {
    	if (appConfig.getCheckSender())
            return  (title.contains(titleKey) || title.contains(titleKey2)) && checkSender(sender);//mailSender.equals(sender);
        else
            return (title.contains(titleKey) || title.contains(titleKey2));
    }
    /**
	 * 校验发送者
	 * 	
