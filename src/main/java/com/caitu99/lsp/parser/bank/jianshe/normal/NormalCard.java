...
import com.caitu99.lsp.parser.bank.jianshe.Jianshe;
 ...
 */
@Service("JiansheNormalCard")
public class NormalCard extends Jianshe implements ICard {
	
	@Autowired
    private AppConfig appConfig;

	@Value("${jianshe.normal.id}")
	private Integer id;

	@Value("${jianshe.normal.name}")
	private String name;

	@Value("${jianshe.normal.bill.type}")
	private int billType;

	@Value("${jianshe.normal.mail.key}")
	private String titleKey;

	@Value("${jianshe.normal.mail.sender}")
	private String mailSender;

	@Value("${jianshe.normal.history.bill}")
