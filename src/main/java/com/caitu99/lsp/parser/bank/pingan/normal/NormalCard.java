...
import com.caitu99.lsp.parser.bank.pingan.Pingan;
import com.caitu99.lsp.parser.utils.BillHelper;
import com.caitu99.lsp.utils.SpringContext;

@Service("PinganNormalCard")
public class NormalCard extends Pingan implements ICard {
	
	@Autowired
    private AppConfig appConfig;

	@Value("${pingan.normal.id}")
	private Integer id;

	@Value("${pingan.normal.name}")
	private String name;

	@Value("${pingan.normal.bill.type}")
	private int billType;

	@Value("${pingan.normal.mail.key}")
	private String titleKey;

	@Value("${pingan.normal.mail.sender}")
	private String mailSender;

	@Value("${pingan.normal.history.bill}")
