...
import com.caitu99.lsp.parser.bank.pufa.Pufa;
import com.caitu99.lsp.parser.utils.BillHelper;
import com.caitu99.lsp.utils.SpringContext;

@Service("PufaNormalCard")
public class NormalCard extends Pufa implements ICard{
	
	@Autowired
    private AppConfig appConfig;

	@Value("${pufa.normal.id}")
	private Integer id;

	@Value("${pufa.normal.name}")
	private String name;

	@Value("${pufa.normal.bill.type}")
	private int billType;

	@Value("${pufa.normal.mail.key}")
	private String titleKey;

	@Value("${pufa.normal.mail.sender}")
	private String mailSender;

	@Value("${pufa.normal.history.bill}")
