...
import com.caitu99.lsp.parser.bank.nongye.Nongye;
 ...
 */
@Service("NongyeNormalCard")
public class NormalCard extends Nongye implements ICard {
	
	@Autowired
    private AppConfig appConfig;

	@Value("${nongye.normal.id}")
	private Integer id;

	@Value("${nongye.normal.name}")
	private String name;

	@Value("${nongye.normal.bill.type}")
	private int billType;

	@Value("${nongye.normal.mail.key}")
	private String titleKey;

	@Value("${nongye.normal.mail.sender}")
	private String mailSender;

	@Value("${nongye.normal.history.bill}")
