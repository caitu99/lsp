...
/**
 * Created by liuzs on 2016/3/31.
 * 广发卡3月起始的账单模板
 */

@Template("guangfa.normal.tpl4")
public class NormalTpl4 extends BaseTpl {
    private final static Logger logger = LoggerFactory.getLogger(NormalTpl4.class);
    ...
    public NormalTpl4(ParserContext context) {
        super(context);
    }
    ...
    public boolean parse() {
        ...
        Element container = DOMHelper.getMaxContainer(tablestop, "感谢您使用广发银行信用卡");
        if( container == null){
            logger.info("未能解析账单,{}", mailSrc);
            return false;
        }
        Elements tables = container.getElementsByTag("table");      //获取到顶层table下所有的table
        //获取用户名
        Pattern namePattern = Pattern.compile("(?<=尊敬的).*(?=(先生|女士),您好！)");
        ...
        //账单月
        Pattern billPattern = Pattern.compile("(?<=以下是您)\\d{4}年\\d{1,2}月(?=的信用卡电子账单)");
        ...
        String beginToEnd = tds.get(2).text().trim().replaceAll(" ", "").replaceAll("\u00A0","");
        Pattern pattern = Pattern.compile("\\d{4}/\\d{2}/\\d{2}(?=至)");
        ...
        pattern = Pattern.compile("(?<=至)\\d{4}/\\d{2}/\\d{2}");
        matcher = pattern.matcher(beginToEnd);
        if(!matcher.find()){
            logger.info("未能解析到账单周期，{}", mailSrc);
            return false;
        ...
        //获取卡号，额度，还款日
        Element accountTbl = DOMHelper.getMinContainer(tables, "信用卡账户信息&本期应还总额");
        if(accountTbl == null){
        ...
        for (int i = 0; i < cnt; i++) {
            ...
            accountMsg.CardNo = fonts.get(i*6+1).text().trim();
            accountMsg.paymentDueDate = fonts.get(i*6+4).text().trim();
            accountMsg.limit =fonts.get(i*6+6).text().trim();
        ...
        fonts = trs.get(2).getElementsByTag("font");
        fontCnt = fonts.size() - 2;
        if (fontCnt < 1 || fontCnt % 6 != 0) {
            logger.info("未能解析到积分,{}",mailSrc);
            return false;
        }
        ...
        if (cnt < 1) {
            ...
            for (int i = 0; i < cnt; i++) {
                String cardNo = fonts.get(i*6+1).text().trim();
                ...
                if (accountMsg != null) {
                    accountMsg.integral = fonts.get(i*6+6).text().trim();
                } else {
                    logger.info("未能解析到积分,{}", mailSrc);
                    return false;
                }
    ...
    private void getAccountMsg(Element element, AccountMsg accountMsg) {
