...
import com.caitu99.lsp.parser.bank.xingye.Xingye;
import com.caitu99.lsp.parser.utils.BillHelper;
import com.caitu99.lsp.utils.SpringContext;

@Service("XingyeNormalCard")
public class NormalCard extends Xingye implements ICard {
	
	@Autowired
    private AppConfig appConfig;

	@Value("${xingye.normal.id}")
	private Integer id;

	@Value("${xingye.normal.name}")
	private String name;

	@Value("${xingye.normal.bill.type}")
	private int billType;

	@Value("${xingye.normal.mail.key}")
	private String titleKey;

	@Value("${xingye.normal.mail.sender}")
	private String mailSender;

	@Value("${xingye.normal.history.bill}")
