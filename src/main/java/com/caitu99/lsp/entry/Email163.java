 ...
 */
@Controller
public class Email163 extends BaseController {
    private static final Logger logger = LoggerFactory.getLogger(Email163.class);
    @Autowired
    private RedisOperate redis;

    @RequestMapping(value = "/api/mail/163/login/1.0", produces = "application/json;charset=utf-8")
    @ResponseBody
    public DeferredResult<Object> login(HttpServletRequest request) {
        ...
        String key = String.format(Constant.MAIL163TASKQUEUE, account);
        ...
        Mail163SpiderEvent event = new Mail163SpiderEvent(userid, deferredResult, request);
        event.setAccount(account);
        event.setPassword(password);
        event.setDate(Long.parseLong(date));
        event.setState(Mail163SpiderState.LOGIN); // the first step is to login
        SpiderReactor.getInstance().process(event);
        logger.info("start processing mail 163 event: {}", event);
        return deferredResult;
    }

    @RequestMapping(value = "/api/mail/163/verify/1.0", produces = "application/json;charset=utf-8")
    @ResponseBody
    public DeferredResult<Object> verify(HttpServletRequest request) {
        ...
        String key = String.format(Constant.mail163ImportKey, userid);
        ...
        Mail163SpiderEvent event = JSON.parseObject(content, Mail163SpiderEvent.class);
        event.setDeferredResult(deferredResult);
        event.setRequest(request);
        event.setvCode(vCode);
        event.setState(Mail163SpiderState.VFY); // verify vcode
    ...
    @RequestMapping(value = "/api/mail/163/checkresult/1.0", produces = "application/json;charset=utf-8")
    @ResponseBody
    public DeferredResult<Object> checkResult(HttpServletRequest request) {
        ...
        String key = String.format(Constant.mail163ResultKey, userid);
        String content = redis.getStringByKey(key);
