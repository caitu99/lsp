 ...
 */
@Controller
public class Email139 extends BaseController {
    private static final Logger logger = LoggerFactory.getLogger(Email139.class);
    @Autowired
    private RedisOperate redis;

    @RequestMapping(value = "/api/mail/139/login/1.0", produces = "application/json;charset=utf-8")
    @ResponseBody
    public DeferredResult<Object> login(HttpServletRequest request) {
        ...
        String key = String.format(Constant.MAIL139TASKQUEUE, account);
        ...
        Mail139SpiderEvent event = new Mail139SpiderEvent(userid, deferredResult, request);
        event.setAccount(account);
        event.setPassword(password);
        event.setDate(Long.parseLong(date));
        event.setState(Mail139SpiderState.LOGINPAGE); // the first step is to login
        SpiderReactor.getInstance().process(event);
        logger.info("start processing mail 139 event: {}", event);
        return deferredResult;
    }

    @RequestMapping(value = "/api/mail/139/verify/1.0", produces = "application/json;charset=utf-8")
    @ResponseBody
    public DeferredResult<Object> verify(HttpServletRequest request) {
        ...
        String key = String.format(Constant.mail139ImportKey, userid);
        ...
        Mail139SpiderEvent event = JSON.parseObject(content, Mail139SpiderEvent.class);
        event.setDeferredResult(deferredResult);
        event.setRequest(request);
        event.setvCode(vCode);
        event.setState(Mail139SpiderState.VFY); // verify vcode
    ...
    @RequestMapping(value = "/api/mail/139/checkresult/1.0", produces = "application/json;charset=utf-8")
    @ResponseBody
    public DeferredResult<Object> checkResult(HttpServletRequest request) {
        ...
        String key = String.format(Constant.mail139ResultKey, userid);
        String content = redis.getStringByKey(key);
