 ...
 * @Copyright (c) 2015-2020 by caitu99
 */
public class CmbChinaBodySpider extends MailBodySpider {

    private final static Logger logger = LoggerFactory.getLogger(CmbChinaBodySpider.class);
    ...
    public void run() {
        while (!isInterrupted()) {
            ...
            logger.debug("cmb china body spider do polling...");
            try {
                event = events.takeFirst();
            } catch (InterruptedException e1) {
                logger.error("cmb china body spider thread is interrupted");
            }
