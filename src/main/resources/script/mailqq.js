var QQ = (function () {
    ...
    var Encryption = function () {
        ...
        function getEncryption(password, salt, vcode, isMd5) {
            ...
            var ttt = rsaH1Len + rsaH1 + window.TEA.strToBytes(salt) + vcodeLen + hexVcode;
            var saltPwd = window.TEA.enAsBase64(rsaH1Len + rsaH1 + window.TEA.strToBytes(salt) + vcodeLen + hexVcode);
            window.TEA.initkey("");
            /*
             * setTimeout(function() { __monitor(488358, 1) }, 0);
             */
...
function getQQEncryption(password, salt, randStr) {
    var decodedSalt = [];
    var s = salt.split("\\");
    for (var i = 1; i < s.length; i++) {
        s[i] = s[i].replace('x', '');
        decodedSalt += String.fromCharCode(parseInt(s[i], 16))
    }
    return QQ.Encryption.getEncryption(password, decodedSalt, randStr, false);
}
